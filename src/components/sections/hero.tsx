...
export function Hero() {
  ...
  useEffect(() => {
    // ✅ Explicitly type your timeout and interval variables
    ...
    const type = () => {
      ...
      const typeLine2 = () => {
        if (j < textLines[1].length) {
          ...
          timeoutId = setTimeout(type, 4000);
        }
      };
  ...
  return (
    <section className="relative py-20 md:py-28">
      ...
      <div className="container relative mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
        <div className="animate-in fade-in slide-in-from-top-8 duration-1000 ease-in-out">
          <h1 className="font-headline text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl md:text-6xl lg:text-7xl min-h-[150px] sm:min-h-[190px] md:min-h-[240px] lg:min-h-[280px] transition-all">
            <span>
              {displayText.line1}
              {showLine1Cursor && showCursor && <span className="text-primary">|</span>}
            </span>
